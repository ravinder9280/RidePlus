...
function primaryEmail(
  d: Pick<UserJSON, "email_addresses" | "primary_email_address_id">,
): string | null {
  const arr = (d?.email_addresses ?? []) as Array<{
    id: string;
  ...
  return (
    arr.find((e) => e.id === pid)?.email_address ??
    arr[0]?.email_address ??
    null
  );
}
