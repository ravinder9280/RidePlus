...
const userButtonAppearance = {
  elements: {
    userButtonAvatarBox: "w-24 h-24", // Example: Custom width and height using Tailwind CSS classes
  },
};
    ...
    <div className="p-2 mx-auto container xl:p-0 flex flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col flex-1">
          <span className="text-2xl gap-1 font-bold ">
            {user?.name || "User"}
          </span>
      ...
      <div className="flex flex-col gap-5">
        <div className="flex items-center justify-between">
          <h4 className="text-lg text-accent-foreground">Posted Rides</h4>
          <Button variant="ghost">
            <Link
              className="text-muted-foreground"
              href={`/user/${user?.id}/rides`}
            >
              See all
            </Link>
          </Button>
        ...
        <div className="grid gap-4 sm:gap-5 md:gap-6 md:grid-cols-2 lg:grid-cols-3 ">
          {user?.rides &&
            user.rides.length > 0 &&
            user.rides.map((ride: Ride) => <RideCard key={ride.id} r={ride} />)}
        </div>
      </div>
    </div>
  );
